...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            {/* Line 1 */}
            <span
              className={`h-0.5 w-full origin-left rounded-full bg-white transition-all duration-500 ${isOpen ? "rotate-45 translate-x-1.25 -translate-y-px" : ""}`}
            ></span>
            {/* Line 2 */}
            <span
              className={`h-0.5 w-full rounded-full bg-white transition-all duration-500 ${isOpen ? "opacity-0 scale-x-0" : ""}`}
            ></span>
            {/* Line 3 */}
            <span
              className={`h-0.5 w-full origin-left rounded-full bg-white transition-all duration-500 ${isOpen ? "-rotate-45 translate-x-1.25 translate-y-1" : ""}`}
            ></span>
          </button>
        </div>
      </nav>
      ...
      {/* Overllay */}
      <div
        className={`z-30 w-full h-screen bg-black/45 fixed top-0 transition-all ease-in-out  duration-1000  ${isOpen ? "" : "hidden"}`}
      ></div>
    </>
  );
}
